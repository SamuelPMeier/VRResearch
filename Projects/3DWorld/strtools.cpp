...
{
	return 0 == _strnicmp( sString.c_str(), sPrefix.c_str(), sPrefix.length() );
}
...
bool StringHasSuffix( const std::string &sString, const std::string &sSuffix )
{
	...
	return 0 == _stricmp( sStringSuffix.c_str(), sSuffix.c_str() );
}
...
// String copy from pchSource to pchBuffer, but no more than unBufferSizeBytes
// counting NUL.  Not quite the same as strncpy_s as it permits truncation
// without complaint.
void strcpy_safe(char *pchBuffer, size_t unBufferSizeBytes, const char *pchSource)
{
	pchBuffer[ unBufferSizeBytes - 1 ] = '\0';  // Ensure termination
